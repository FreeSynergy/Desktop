    ...
    rsx! {
        div {
            ...
            div {
                ...
                div {
                    style: "padding: 12px; border-bottom: 1px solid var(--fsn-color-border-default); \
                    ...
                    div { style: "display: flex; gap: 6px;",
                        button {
                            ...
                            style: "flex: 1; background: var(--fsn-color-primary); color: #fff; \
...
fn TaskListRow(task: TaskPipeline, is_active: bool, on_click: EventHandler<MouseEvent>) -> Element {
    ...
    let border = if is_active { "2px solid var(--fsn-color-primary)" } else { "2px solid transparent" };
    ...
    rsx! {
        div {
            ...
            div { style: "flex: 1; min-width: 0;",
                div {
                    style: "font-size: 13px; font-weight: 500; color: var(--fsn-color-text-primary); \
...
fn TaskDetail(
    ...
    rsx! {
        div { style: "display: flex; flex-direction: column; gap: 20px;",
            ...
            div {
                h2 { style: "margin: 0 0 4px; font-size: 18px; color: var(--fsn-color-text-primary);",
                    "{task.name}"
                }
            ...
            div {
                style: "display: flex; align-items: center; gap: 16px; \
                        ...
                        border-radius: var(--fsn-radius-md); padding: 16px;",
                div {
                    div { style: "font-size: 10px; text-transform: uppercase; color: var(--fsn-color-text-muted);", "Source" }
                    div { style: "font-size: 14px; font-weight: 600; color: var(--fsn-color-text-primary);", "{task.source.service}" }
                    div { style: "font-size: 12px; color: var(--fsn-color-primary);", "{task.source.offer}" }
                }
                div { style: "font-size: 20px; color: var(--fsn-color-primary); flex: 1; text-align: center;", "──▶" }
                div {
                    div { style: "font-size: 10px; text-transform: uppercase; color: var(--fsn-color-text-muted);", "Target" }
                    div { style: "font-size: 14px; font-weight: 600; color: var(--fsn-color-text-primary);", "{task.target.service}" }
                    div { style: "font-size: 12px; color: var(--fsn-color-primary);", "{task.target.accept}" }
                }
            }
            ...
            if !task.mappings.is_empty() {
                div {
                    div { style: "font-size: 12px; font-weight: 600; text-transform: uppercase; \
                                  letter-spacing: 0.06em; color: var(--fsn-color-text-muted); margin-bottom: 8px;",
                        "Field Mapping"
                    }
                        ...
                        for (i, m) in task.mappings.iter().enumerate() {
                            div {
                                ...
                                style: "display: flex; align-items: center; gap: 8px; \
                                        ...
                                        border-radius: var(--fsn-radius-sm); \
                                        padding: 6px 10px; font-size: 12px;",
                                span { style: "color: var(--fsn-color-text-muted); min-width: 120px;",
                                    "{m.source_field.as_deref().unwrap_or(\"(fixed)\")}"
                                }
                                span { style: "color: var(--fsn-color-primary);", "→" }
                                span { style: "color: var(--fsn-color-text-primary);", "{m.target_field}" }
                                span { style: "color: var(--fsn-color-text-muted); font-size: 11px; margin-left: auto;",
                                    "{m.transform.label()}"
                                }
                            }
                        }
            ...
            div {
                div { style: "font-size: 12px; font-weight: 600; text-transform: uppercase; \
                              letter-spacing: 0.06em; color: var(--fsn-color-text-muted); margin-bottom: 4px;",
                    "Trigger"
                }
                span { style: "font-size: 13px; color: var(--fsn-color-text-primary);", "{trigger_label}" }
            }
            ...
            div { style: "display: flex; gap: 8px;",
                ...
                button {
                    ...
                    style: "background: var(--fsn-color-bg-overlay); \
                            ...
                            color: var(--fsn-color-text-primary); cursor: pointer;",
                    "✏ Edit"
                }
                button {
...
fn TemplateBrowser(on_use_template: EventHandler<&'static str>) -> Element {
    rsx! {
        div { style: "display: flex; flex-direction: column; gap: 12px;",
            h3 { style: "margin: 0; font-size: 16px; color: var(--fsn-color-text-primary);",
                "Task Templates"
            }
...
fn TemplateCard(
    ...
    rsx! {
        div {
            ...
            p { style: "margin: 0; font-size: 12px; color: var(--fsn-color-text-muted);",
