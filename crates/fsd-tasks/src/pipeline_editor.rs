        ...
        div { style: "display: flex; flex-direction: column; gap: 16px;",
            ...
            div {
                ...
                input {
                    ...
                    style: "width: 100%; background: var(--fsn-color-bg-overlay); \
            ...
            div { style: "display: flex; align-items: center; gap: 16px;",
                ...
                span { style: "font-size: 20px; color: var(--fsn-color-primary);", "──▶" }
...
fn ServiceBox(label: &'static str, service: String, offer: String) -> Element {
    rsx! {
        div {
            style: "flex: 1; background: var(--fsn-color-bg-overlay); \
                    border: 1px solid var(--fsn-color-border-default); \
                    border-radius: var(--fsn-radius-md); padding: 12px;",
            div {
            ...
            div { style: "font-size: 13px; font-weight: 600; color: var(--fsn-color-text-primary);", "{service}" }
            div { style: "font-size: 12px; color: var(--fsn-color-primary); margin-top: 2px;", "{offer}" }
        }
    }
}
    ...
    rsx! {
        div {
            style: "display: grid; grid-template-columns: 1fr auto 1fr; gap: 8px; \
                    align-items: center; background: var(--fsn-color-bg-overlay); \
                    border-radius: var(--fsn-radius-sm); padding: 6px 10px; font-size: 12px;",
            span { style: "color: var(--fsn-color-text-primary);", "{source}" }
            span { style: "color: var(--fsn-color-primary); font-size: 14px;", "→" }
            span { style: "color: var(--fsn-color-text-primary);", "{mapping.target_field}" }
        }
    }
}
...
fn TriggerOption(label: &'static str, selected: bool, on_select: EventHandler<()>) -> Element {
    ...
    let color = if selected { "var(--fsn-color-primary)" } else { "var(--fsn-color-text-muted)" };
    rsx! {
        div {
