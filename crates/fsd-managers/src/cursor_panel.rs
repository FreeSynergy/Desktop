...
use dioxus::prelude::*;
use fsn_manager_cursor::{CursorManager, CursorRepository, CursorSet, CursorSetDraft, CursorSlot};
...
pub fn CursorManagerPanel() -> Element {
    let icons_root = std::path::PathBuf::from(
        std::env::var("FSN_ICONS_ROOT").unwrap_or_else(|_| "../FreeSynergy.Icons".into()),
    );
                ...
                match view.read().clone() {
                    View::SetDetail(id) => rsx! {
                        SetDetailView {
                            ...
                            set_id: id.clone(),
                            active_id: active.read().clone(),
                            icons_root: std::env::var("FSN_ICONS_ROOT")
        ...
        div { style: "{sidebar_style}",
                    ...
                    div {
                        ...
                        div {
                            style: "font-size: 10px; font-weight: 600; letter-spacing: 0.05em; \
                                    text-transform: uppercase; color: var(--fsn-color-primary, #06b6d4); \
                                    margin-bottom: 4px;",
                            "Active Set"
                        }
                        div {
                    ...
                    for set in &sets {
                        {
                            ...
                            let bg = if is_sel {
                                "background: var(--fsn-sidebar-active-bg, rgba(6,182,212,0.15)); \
            ...
            // Settings (pinned bottom)
            {
                ...
                let bg = if is_sel {
                    ...
                    "background: transparent; color: var(--fsn-color-text-muted);"
                };
                rsx! {
                    div {
                        style: "border-top: 1px solid var(--fsn-color-border-default, #334155);",
                        div {
                            style: "display: flex; align-items: center; gap: 10px; \
...
fn SetDetailView(
    ...
    let Some(set) = sets.iter().find(|s| s.id == set_id).cloned() else {
        return rsx! {
            div { style: "padding: 40px; color: var(--fsn-color-text-muted);",
                "Set not found."
            }
        };
    };
        ...
        div { style: "padding: 24px 28px; max-width: 720px;",
            ...
            div {
                ...
                div {
                    ...
                    div {
                        style: "margin-top: 6px; font-size: 12px; color: var(--fsn-color-text-muted);",
                ...
                if !is_active {
                    button {
                        style: "padding: 8px 20px; font-size: 13px; font-weight: 600; \
                                background: var(--fsn-color-primary, #06b6d4); color: #fff; \
            ...
            div {
                style: "margin-bottom: 24px; padding: 16px; \
                        background: var(--fsn-color-bg-overlay, #1e293b); \
                        border-radius: var(--fsn-radius-md, 6px);",
                div {
                    ...
                    for slot in &preview_slots {
                        {
                            ...
                            rsx! {
                                div {
                                    ...
                                    span {
                                        style: "font-size: 10px; color: var(--fsn-color-text-muted); \
                ...
                for slot in CursorSlot::all() {
                    {
                        ...
                        rsx! {
                            div {
                                ...
                                span {
                                    style: "font-size: 9px; color: var(--fsn-color-text-muted); \
...
fn NewSetView(
    ...
    rsx! {
        div { style: "padding: 24px 28px; max-width: 680px;",
            ...
            p {
                style: "margin: 0 0 24px; font-size: 13px; color: var(--fsn-color-text-muted);",
            ...
            if !error.read().is_empty() {
                div {
                    style: "margin-bottom: 16px; padding: 10px 14px; font-size: 13px; \
                            ...
                            border-radius: var(--fsn-radius-md, 6px); color: #ef4444;",
                    "{error}"
                }
            }
            ...
            div {
                style: "margin-bottom: 4px; display: grid; \
                        ...
                        background: var(--fsn-color-bg-overlay, #1e293b); \
                        border-radius: var(--fsn-radius-md, 6px) var(--fsn-radius-md, 6px) 0 0;",
                span { style: "font-size: 11px; color: var(--fsn-color-text-muted); font-weight: 600;", "Slot" }
                span { style: "font-size: 11px; color: var(--fsn-color-text-muted); font-weight: 600;", "SVG Path" }
                span { style: "font-size: 11px; color: var(--fsn-color-text-muted); font-weight: 600;", "Hotspot X" }
                span { style: "font-size: 11px; color: var(--fsn-color-text-muted); font-weight: 600;", "Hotspot Y" }
            }
            div {
                style: "border: 1px solid var(--fsn-color-border-default, #334155); \
                        border-radius: 0 0 var(--fsn-radius-md, 6px) var(--fsn-radius-md, 6px); \
                        overflow: hidden;",
...
fn SettingsView() -> Element {
    ...
    rsx! {
        div { style: "padding: 24px 28px; max-width: 560px;",
            ...
            p {
                style: "margin: 0 0 20px; font-size: 13px; color: var(--fsn-color-text-muted);",
            ...
            div {
                style: "border: 1px solid var(--fsn-color-border-default, #334155); \
                        border-radius: var(--fsn-radius-md, 6px); overflow: hidden;",
                div {
        ...
        div {
            label {
                style: "display: block; font-size: 12px; font-weight: 500; \
                        color: var(--fsn-color-text-muted); margin-bottom: 5px;",
                "{label}"
            }
            {children}
        }
...
const INPUT_STYLE: &str =
    "width: 100%; box-sizing: border-box; padding: 7px 10px; font-size: 13px; \
     ...
     border: 1px solid var(--fsn-color-border-default, #334155); \
     border-radius: var(--fsn-radius-md, 6px); \
     color: var(--fsn-color-text-primary); outline: none;";
