...
/// universal FsnTabView slide+blur animation.
use dioxus::prelude::*;
use fsn_components::{FsnSidebar, FsnSidebarItem, FsnTabDef, FsnTabView,
                     FSN_SIDEBAR_CSS, FSN_TAB_VIEW_CSS};
...
/// The active section switches via FsnTabView with a slide+blur animation.
...
impl StoreSection {
    ...
    pub fn label(&self) -> String {
        match self {
            Self::Server  => fsn_i18n::t("store.section_tab.server"),
            Self::Apps    => fsn_i18n::t("store.section_tab.apps"),
            Self::Desktop => fsn_i18n::t("store.section_tab.desktop"),
        }
    }
    ...
    pub fn sidebar_items(&self) -> Vec<FsnSidebarItem> {
        match self {
            ...
            Self::Apps => vec![
                FsnSidebarItem::new("bundles",  ICON_BUNDLE, fsn_i18n::t("store.sidebar.bundles")),
                FsnSidebarItem::new("apps",     ICON_APPS,   fsn_i18n::t("store.sidebar.apps")),
                FsnSidebarItem::new("bots",     ICON_BOT,    fsn_i18n::t("store.sidebar.bots")),
                FsnSidebarItem::new("installed",ICON_INSTALLED, fsn_i18n::t("store.sidebar.installed")),
            ...
            Self::Desktop => vec![
                ...
                FsnSidebarItem::new("icons",    ICON_ICONS,   fsn_i18n::t("store.sidebar.icons")),
                FsnSidebarItem::new("languages",ICON_LANG,    fsn_i18n::t("store.sidebar.languages")),
                FsnSidebarItem::new("installed",ICON_INSTALLED, fsn_i18n::t("store.sidebar.installed")),
                FsnSidebarItem::new("updates",  ICON_UPDATES, fsn_i18n::t("store.sidebar.updates")),
            ],
        }
...
/// ```
/// ┌─────────────────────────────────────────────────────┐
/// │  [Server]  [Apps]  [Desktop]     ← FsnTabView bar   │
/// ├──────────────────────────────────────────────────────┤
/// │  ┌───────────┐  ┌────────────────────────────────┐  │
/// │  │ Bundles   │  │  search …                      │  │
/// │  │ Server    │  │                                │  │
...
pub fn StoreApp() -> Element {
    ...
    let section_tabs: Vec<FsnTabDef> = vec![
        FsnTabDef::new(StoreSection::Server.id(),  StoreSection::Server.label())
            .with_icon(ICON_SERVER.to_string()),
        FsnTabDef::new(StoreSection::Apps.id(),    StoreSection::Apps.label())
            .with_icon(ICON_APPS.to_string()),
        FsnTabDef::new(StoreSection::Desktop.id(), StoreSection::Desktop.label())
            .with_icon(ICON_DESKTOP.to_string()),
    ];

    let sidebar_items = cur_section.sidebar_items();
    let pinned = vec![
        FsnSidebarItem::new("settings", ICON_SETTINGS, fsn_i18n::t("store.sidebar.settings")),
    ];
        ...
        div {
            ...
            if !catalog_versions.is_empty() {
                p { style: "margin-top: 16px; font-size: 13px;",
                    {fsn_i18n::t_with("store.updates.catalog_count",
